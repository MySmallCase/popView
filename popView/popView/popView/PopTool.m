...
+ (PopTool *)sharedInstance {
    static PopTool *sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedClient = [[PopTool alloc]init];
    });
    return sharedClient;
}
...
- (void)setCloseButtonType:(ButtonPositionType)closeButtonType {
    ...
    if (closeButtonType == ButtonPositionTypeNone) {
        ...
        if(closeButtonType == ButtonPositionTypeRight){
            closeFrame.origin.x = round(CGRectGetWidth(self.containerView.frame) - kDefaultCloseButtonPadding - CGRectGetWidth(closeFrame)) - 4;
...
//    [containerView addSubview:presentView];
    ...
    if(self.closeButtonType == ButtonPositionTypeRight){
        ...
        closeFrame.origin.y = CGRectGetMinY(presentView.bounds) - CGRectGetHeight(closeFrame) + padding + 4;
...
- (void)drawRect:(CGRect)rect{
    ...
    if ([PopTool sharedInstance].shadeBackgroundType == ShadeBackgroundTypeSolid) {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if ([PopTool sharedInstance].tapOutsideToDismiss) {
        [[PopTool sharedInstance] hideAnimated:YES withCompletionBlock:nil];
    }
}
...
- (instancetype)init {
    ...
    self = [super initWithFrame:(CGRect){0, 0, 44, 42}];
    if (self) {
        [self setBackgroundImage:[UIImage imageNamed:@"close"] forState:UIControlStateNormal];
        [self setBackgroundImage:[UIImage imageNamed:@"close"] forState:UIControlStateHighlighted];
    }
    return self;
}
